...
def load_data(cfg, root_path):
    ...
    system_table = pd.read_csv(str(root_path / cfg.system_table_path))
...
def preprocess(cfg, train_fold_df, valid_fold_df, st_df):
    ...
    # station_dfから値をとる
    st2system_df = st_df[['id', 'system']]
    st2system_df['id'] = st2system_df['id'].astype(str)
    st2system = st2system_df.set_index('id').to_dict()['system']

    st2info = {st: {'mean': st2mean[st], 'std': st2std[st], 'system': st2system[st]} for st in st2mean.keys()}
...
class HiroshimaDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        meta['system'] = self.st2info[station]['system']
...
class Encoder(nn.Module):
    def __init__(self, input_size, hidden_size, system_embed):
        super().__init__()
        self.lstm = nn.LSTM(input_size=input_size+system_embed[1], hidden_size=hidden_size, batch_first=True)
        self.system_embeddings = nn.Embedding(system_embed[0], system_embed[1])
    
    def forward(self, x, system, h0=None):
        '''
            x: (bs, input_seq_size ,input_size)
            h0 = Tuple(h, c)
            st: (bs, 1)
        '''
        system = system.repeat(1, x.size()[1]) # (bs, len_of_series)
        system_embed = self.system_embeddings(system) # (bs, input_seq_size, station_embed_size)
        x = torch.cat([x, system_embed], axis=-1)
...
class Model(nn.Module):
    def __init__(self, cfg, n_systems, device):
        super().__init__()
        ...
        self.device = device
        self.systems_embed = [n_systems, cfg.system_embed_size]
        self.encoder = Encoder(self.input_size, self.hidden_size, self.systems_embed)
        self.decoder = Decoder(self.hidden_size, self.output_size)
    
    def forward(self, x, target, system, teacher_forcing_ratio):
        encoder_state = self.encoder(x, system, h0=None)
        decoder_input = x[:, -1:, :]
...
def train_one_epoch(cfg, epoch, dataloader, model, loss_fn, device, optimizer, scheduler, scheduler_step_time):
    ...
    for step, (data, target, meta) in pbar:
        ...
        systems = torch.tensor(list(map(int, meta['system']))).unsqueeze(-1).to(device).long()
        pred = model(data, target, systems, teacher_forcing_ratio).squeeze() 
...
def valid_one_epoch(cfg, epoch, dataloader, model, loss_fn, device):
    ...
    for step, (data, target, meta) in pbar:
        ...
        with torch.no_grad():
            pred = model(data, target, systems, 0.).squeeze()
...
def main():
    ...
    df, st_df, system_table = load_data(cfg, root_path)
    ...
    for fold in range(cfg.n_folds):
        ...
        train_fold_df, valid_fold_df, st2info = preprocess(cfg, train_fold_df, valid_fold_df, st_df)
        train_loader, valid_loader = prepare_dataloader(cfg, train_fold_df, valid_fold_df, st2info)

        model = Model(cfg, n_systems, device).to(device)
