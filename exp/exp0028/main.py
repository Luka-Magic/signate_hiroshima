...
def preprocess(cfg, train_fold_df, valid_fold_df):
    ...
    ## train_fold_dfの標準化
    data_mean, data_std = np.nanmean(train_data.values), np.nanstd(train_data.values)
    print(f'debug: data_mean: {data_mean}, data_std: {data_std}')
    train_zscore_data = (train_data - data_mean) / data_std
    ...
    valid_zscore_df = (valid_data - data_mean) / data_std
    ...
    st2info = {st: {'mean': data_mean, 'std': data_std} for st in st2mean.keys()}
