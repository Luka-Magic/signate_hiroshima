...
class Decoder(nn.Module):
    ...
    def forward(self, x, h):
        '''
            ...
            h = Tuple(h, c)
        '''
        x, _ = self.lstm(x, h) # -> x: (bs, len_of_series, hidden_size)
...
def train_one_epoch(cfg, epoch, dataloader, encoder, decoder, loss_fn, device, encoder_optimizer, decoder_optimizer, encoder_scheduler, decoder_scheduler, scheduler_step_time):
    ...
    for step, (data, target, meta) in pbar:
        ...
        if cfg.bidirectional:
            onedir_h = h.permute(1, 0, 2).contiguous().view(1, -1, cfg.hidden_size*2)
            repeat_input = onedir_h.transpose(1, 0).repeat(1, cfg.output_sequence_size, 1) # repeat_input: (bs, len_of_series, hidden_size) # repeat_input: (bs, len_of_series, hidden_size)
...
def valid_one_epoch(cfg, epoch, dataloader, encoder, decoder, loss_fn, device):
    ...
    for step, (data, target, meta) in pbar:
        ...
        with torch.no_grad():
            ...
            if cfg.bidirectional:
                onedir_h = h.permute(1, 0, 2).contiguous().view(1, -1, cfg.hidden_size*2)
                repeat_input = onedir_h.transpose(1, 0).repeat(1, cfg.output_sequence_size, 1) # repeat_input: (bs, len_of_series, hidden_size)
            pred = decoder(repeat_input, (h, c)).squeeze() # pred: (bs, len_of_series, output_size)
