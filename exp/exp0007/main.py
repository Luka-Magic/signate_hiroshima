...
class HiroshimaDataset(Dataset):
    def __init__(self, cfg, df, st2info, phase):
        ...
        for border in tqdm(range(start_row, last_row, 24)):
            ...
            input_ = df.iloc[max(border-self.input_sequence_size, 0):border, :].drop(columns=['date', 'hour'])
            ...
            self.border_idx += [self.first_index + border]*len(target.columns)
    ...
    def __getitem__(self, idx):
        ...
        meta['border'] = self.border_idx[idx]
