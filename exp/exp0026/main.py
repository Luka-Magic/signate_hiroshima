...
def preprocess(cfg, train_fold_df, valid_fold_df, st_df):
    ...
    # station_dfから値をとる
    st2river_df = st_df[['id', 'river']]
    st2river_df['id'] = st2river_df['id'].astype(str)
    st2river = st2river_df.set_index('id').to_dict()['river']
...
class Encoder(nn.Module):
    def __init__(self, input_size, hidden_size, river_embed):
        super().__init__()
        self.lstm = nn.LSTM(input_size=input_size+river_embed[1], hidden_size=hidden_size, batch_first=True)
        self.river_embeddings = nn.Embedding(river_embed[0], river_embed[1])
    
    def forward(self, x, river, h0=None):
        '''
            x: (bs, input_seq_size ,input_size)
            h0 = Tuple(h, c)
            st: (bs, 1)
        '''
        river = river.repeat(1, x.size()[1]) # (bs, len_of_series)
        river_embed = self.river_embeddings(river) # (bs, input_seq_size, station_embed_size)
        x = torch.cat([x, river_embed], axis=-1)
...
class Model(nn.Module):
    def __init__(self, cfg, n_rivers, device):
        super().__init__()
    ...
    def forward(self, x, target, river, teacher_forcing_ratio):
        encoder_state = self.encoder(x, river, h0=None)
        decoder_input = x[:, -1:, :]
...
def valid_one_epoch(cfg, epoch, dataloader, model, loss_fn, device):
    ...
    for step, (data, target, meta) in pbar:
        ...
        with torch.no_grad():
            pred = model(data, target, rivers, 0.).squeeze()
...
def main():
    ...
    for fold in range(cfg.n_folds):
        ...
        model = Model(cfg, n_rivers, device).to(device)
