...
def rain_process1(rain, rain_st):
    # 1. 全体でdrop_duplicates
    rain.drop_duplicates(inplace=True)
    

    # 2. (date, statition, city)が重複している行 -> その行の中でfloatが最も含まれている行を採用
    ...
    # 4. station名に(砂防)が含まれているものは入力時使用も0であり、ないものとマージできる
    ...
    # 一連の処理で入力時使用がnanになるものがいくつか出てくるので0で埋める。
